...
// import rawCountries from './rawCountriesRu';
...
export function initCountries() {
    return rawCountries.map((country) => ({
        ...
        iso2     : country[2],
        dialCode : country[3],
        format   : country[4] || getDefaultFormat({
            dialCode : country[3]
        }),
...
function getDefaultFormat({ dialCode = '' } = {}) {
    if (!dialCode) return null;
    let result = '+';

    dialCode.split('').forEach(() => result += '.');
...
//     if (typeof enableAreaCodes === 'boolean') {
//         enableAllCodes = true;
//     } else {
...
//                 const country = sourceCountryList.find(country => country.iso2 === countryCode);    // eslint-disable-line
