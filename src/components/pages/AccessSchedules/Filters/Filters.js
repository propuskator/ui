...
function Filters(props) {    /* eslint-disable-line max-lines-per-function */
    ...
    const { createStart, createEnd, updateStart, updateEnd, enabled, periodicity } = filters;
    ...
    return (
        <PageFilters
            ...
            searchLabel    = {t('filters:Find by name')}
            filters        = {filters}
            ...
            defaultExpanded={useMemo(() => (
                !!(createStart || createEnd
                    || updateStart || updateEnd || enabled || periodicity)
            ), [])}
        />
    );
}
